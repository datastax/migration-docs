...
=== Golang (Automation utility?) metrics dashboard
...
The ZDM Automation utility does not provide a way to perform scaling up/down operations. Instead, we recommend that you deploy a new proxy cluster on the side, and move the client applications to the new proxy cluster (if absolutely necessary).
