...
Validation checks are an additional way to verify that all the data has been migrated successfully. For data written by indempotent writes these checks are optional, as any errors, timeouts or other failures during the migration are made visible by the data migrator and also by the ZDM proxy. However, they can provide extra peace of mind.
