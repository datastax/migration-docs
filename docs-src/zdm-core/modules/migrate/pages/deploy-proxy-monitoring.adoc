...
How can you confirm that the ZDM proxies are up and running? Here are two options:
...
[source,bash]
----
GET "http://{{ hostvars[inventory_hostname]['ansible_default_ipv4']['address'] }}:{{ metrics_port }}/health/readiness"
----

The default port for metrics collection is `14001`. Optionally, you can specify a custom, non-default port that was set via a configuration change. See the section on xref:deploy-proxy-monitoring.adoc#_cluster_and_core_configuration[contact points and port].

Readiness example:

[source,bash]
----
http://172.18.10.40:14001/health/readiness
----
