...
G. `read_mode`: leave to its default value of `PRIMARY_ONLY`. lobal see xref:enable-async-dual-reads.adoc[] for more information on this parameter.
...
[TIP]
====
For Grafana dashboard examples and details about the metrics you can observe, see xref:migration-troubleshooting-tips.adoc#how-to-leverage-metrics[this section] of the troubleshooting tips.
====
