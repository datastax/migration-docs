...
At this time, {company} {zdm-product} does not publish releases in binary format. The only release artifact that we publish is the docker image. We plan to publish the binary files on the Releases section of the Github repository soon.
