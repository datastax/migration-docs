...
Check the credentials that are being used by looking up the `vars/monitoring_config.yml` file on the {zdm-automation} directory. These credentials can also be modified before deploying the metrics stack.
