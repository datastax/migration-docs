...
We encourage existing 2.x ZDM users to upgrade to the 2.2.0 version of {zdm-automation}. To do so, simply `git pull` from the `main` branch of https://github.com/datastax/zdm-proxy-automation. 

For more, see xref:deploy-proxy-monitoring.adoc#_configure_the_zdm_proxy[Configure the ZDM Proxy].
...
* xref:feasibility-checklists.adoc#_read_only_applications[Read-only applications, window="_blank"]. See the notes indicating that this issue is solved by the {zdm-proxy} 2.1.0 release.
