...
[NOTE]
====
* Scenario: If you have 20 TBs of existing data to be migrated and want to speed up the migration, you can use (for example) 4 VMs that are the equivalent of an AWS m5.4xlarge, a GCP e2-standard-16 or an Azure D16v5. Then run DSBulk Migrator or Cassandra-Data-Migrator in parallel on each VM, with each one responsible for migrating around 5TB of data. If there is one super large table (e.g. 15 TB of 20 TB is in one table), you can choose to migrate this table in 3 parts on 3 separate VMs in parallel by splitting the full token range into 3 parts and migrate rest of the tables on the 4th VM. 
