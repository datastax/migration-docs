...
[NOTE]
====
Scenario: when you have close to 12 TB's data and n number of tables, to speed up migration of your existing data, you can run with (for example) 4 machines equivalent of AWS `m5.4xlarge` and either run dsbulk-migrator on each machine with each one responsible for a quarter of the full token range, or leverage the parallelism from a spark job to run the migration process across all 4 machines.
====
