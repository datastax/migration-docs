...
== Configure Docker offline

There are two options for users who are not connecting servers directly to the internet.
...
Local registries not connected to the internet require administrators to manually add containers to their registry. For zdm-util users, this option requires the following five containers to install and configure the jump host, ZDM Proxy, and monitoring:
