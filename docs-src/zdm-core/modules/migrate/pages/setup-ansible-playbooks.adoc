...
[NOTE]
====
It is highly recommended to configure a monitoring instance, if you don't have a monitoring stack. For migration that may run for multiple days, it is essential to use metrics to understand the performance and health of the {zdm-proxy} instances.
====
+
