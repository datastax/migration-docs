...
Here's an overview of the migration phases, which matches the structure of this ZDM documentation.

image::{imagesprefix}migration-all-phases.png[Migration phases from start to finish.]

== Interactive walk-through of the migration phases
...
--
.Introduction
To get acquainted with the migration phases from start to finish, try this interactive walk-through presentation. Click the **Start** button below!

image:migration-introduction9.png[Introductory page prompts you to click Start to begin the graphical presentation.]
--

[.slide]
--
.Phase 0: Before your migration starts
Let's look at a pre-migration, high-level view. At this point, your client applications are performing read/write operations with an existing CQL-compatible database. That is, Apache Cassandra®, DataStax Enterprise (DSE), or Astra DB.

image:pre-migration0ra9.png[A pre-migration high-level diagram. Your client applications perform read/write operations with an existing CQL-compatible database: Apache Cassandra®, DataStax Enterprise (DSE), or Astra DB.]
--

[.slide] 
--
.Phase 1: Deploy ZDM Proxy and connect client applications
In this first phase, we'll deploy the ZDM Proxy instances and connect client applications to the proxies. This step activates the dual-write logic. Writes will be "bifurcated" (sent to both Origin and Target), while reads will be executed on Origin only.

image:migration-phase1ra9.png[Migration Phase 1. We deploy ZDM Proxy instances and connect client applications to the proxies. This step activates the dual-write logic. Writes will be "bifurcated" (sent to both Origin and Target), while reads will be executed on Origin only.]
--
...
image:migration-phase2ra9.png[Migration Phase 2. We migrate existing data using Cassandra Data Migrator and/or DSBulk Migrator. Validate that the migrated data is correct, while continuing to perform dual writes.]
--
...
image:migration-phase3ra9.png[Migration Phase 3. Optionally enable asynchronous dual reads. Test performance and verify that Target can handle your application's live request load before cutting over from Origin to Target.
]
--
...
image:migration-phase4ra9.png[Migration Phase 4. Read routing on ZDM Proxy is switched to Target so all reads are executed on it, while writes are still sent to both clusters. In other words, Target becomes the primary cluster.]
--
...
image:migration-phase5ra9.png[Migration Phase 5. Move your client applications off the ZDM Proxy and connect the apps directly to Target. Once that happens, the migration is complete.]
--
====
