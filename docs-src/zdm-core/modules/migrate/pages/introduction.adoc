...
{zdm-shortproduct} provides a simple and reliable way for you to migrate applications from any CQL-based cluster (Apache Cassandra&reg;, {company} Enterprise (DSE), Astra DB, or any type of CQL-based database) to any other CQL-based cluster, without any interruption of service to the client applications and data.

* You can move your application to Astra DB, DSE, or Cassandra with no downtime and with minimal configuration changes. For more, see <<migration-workflow, migration workflow>>.
...
* **Target:** This cluster is the new environment to which you want to migrate apps and data with zero downtime.
...
* Migrate your data from any CQL-supported Origin to any CQL-supported Target. Examples of databases that support CQL are Apache Cassandra, {company} Enterprise and Astra DB.
