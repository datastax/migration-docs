...
Also refer to the per-property configuration details in this topic:
...
| 
| Default is empty. If column names are changed between Origin and Target, then this map-like list provides a mechanism to associate the two. The format is `<origin_column_name>:<target_column_name>`. The list is comma-separated. You only need to list renamed columns. 
...
For information about invoking `DiffData` in a {cstar-data-migrator} command, see xref:#cstar-validation-steps[{cstar-data-migrator} steps in validation mode] in this topic.
