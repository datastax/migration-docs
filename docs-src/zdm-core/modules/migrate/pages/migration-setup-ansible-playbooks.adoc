= Setup Ansible automation playbooks with {zdm-automation}

This topic explains how to use the {zdm-automation} to setup Ansible playbooks that will automate launching a dedicated deployment of the {zdm-proxy} and its companion monitoring stack.

Then continue the steps in the next topic, xref:migration-run-ansible-playbooks.adoc[], to run the playbooks and optionally monitor metrics.
