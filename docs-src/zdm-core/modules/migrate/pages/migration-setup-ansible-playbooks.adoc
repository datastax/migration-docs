= Set up the Ansible automation with {zdm-automation}

This topic explains how to use the {zdm-automation} to set up the Ansible playbooks that will automate launching a dedicated deployment of the {zdm-proxy} and its companion monitoring stack.

After you have done this, the xref:migration-run-ansible-playbooks.adoc[next topic] will show you to use the automation to deploy the {zdm-proxy} and the monitoring stack.
...
[TIP]
====
To simplify accessing the jumphost and {zdm-proxy} instances from your machine, create a custom SSH configuration file as explained xref:migration-deployment-infrastructure.adoc#create_zdm_ssh_config_file[here].
...
Now that you've completed the Ansible setup with {zdm-automation}, learn how to xref:migration-run-ansible-playbooks.adoc[].
