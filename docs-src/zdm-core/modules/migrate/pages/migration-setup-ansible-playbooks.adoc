= Setup Ansible playbooks with ZDM Utility

This topic explains how to use the {zdm-automation} to setup an Ansible playbook that will automate launching a dedicated deployment of the {zdm-proxy} and its companion monitoring stack.

See the next topic, xref:migration-run-ansible-playbooks.adoc[] to run the playbook and optionally monitor metrics.

At the end, you will have a working and fully monitored {zdm-proxy} deployment.
...
Now that you've completed the Ansible setup, learn how to xref:migration-run-ansible-playbooks.adoc[run the Ansible playbook] with the ZDM Utility.
