...
[NOTE]
This issue is fixed in ZDM Proxy v2.1.0, see the Fix section below.
...
This issue has been fixed in ZDM Proxy v2.1.0, we encourage you to upgrade to that version or greater. The proxy will now send heartbeats, every 30 seconds by default, through the idle connection to keep it alive. The heartbeat interval can be tuned with the ZDM_HEARTBEAT_INTERVAL_MS property.
