...
When deployed through the {zdm-automation}, each {zdm-proxy} instance runs as a Docker container.

You can view its logs by connecting to the {zdm-proxy} machine and running the following command:
[source,bash]
----
sudo docker container logs zdm-proxy-container
----

You can also retrieve logs of all {zdm-proxy} instances using a dedicated playbook, as explained xref:manage-proxy-instances.adoc#_collect_the_logs[here].
...
** Read Failure Rate: one `cluster` label with two settings: `origin` and `target`. The label that contains data depends on the `ZDM_PRIMARY_CLUSTER` setting (same as the latency and throughput metrics explained above).
