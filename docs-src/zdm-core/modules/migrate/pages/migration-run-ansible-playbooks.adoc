...
These two sections are very important and are always required. Populate the appropriate parameters in each section for the respective cluster, by uncommenting them and specifying their values as follows:
...
.. If you wish to manually provide the cluster's Secure Connect Bundle, copy it to the container and specify its path in `*_astra_secure_connect_bundle_path`.
