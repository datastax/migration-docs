...
[TIP]
====
If you performed the optional steps to enable async dual reads -- to verify that your Target cluster is ready and tuned appropriately to handle the production read load -- be sure to disable async dual reads when you're done testing. That is, now revert `read_mode` in `vars/zdm_proxy_core_config.yml` to `PRIMARY_ONLY` when switching sync reads to Target. Example:
...
See xref:change-read-routing.adoc[] for the next phase of your migration, as you prepare Target to become the **primary** cluster. 
