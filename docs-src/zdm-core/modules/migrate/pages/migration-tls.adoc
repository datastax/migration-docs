...
* If your TLS files are in a JKS keystore, extract them as plain text (see xref:migration-tls.adoc#_retrieving_files_from_a_jks_keystore[]])
...
* From this shell, uncomment and edit the relevant values in the configuration file `zdm-proxy-automation/ansible/vars/zdm_proxy_custom_tls_config.yml` as follows.
...
Just go back to xref:migration-deploy-proxy-monitoring.adoc#_advanced_configuration_optional[Optional advanced configuration] to finalize the {zdm-proxy} configuration and then execute the deployment playbook.