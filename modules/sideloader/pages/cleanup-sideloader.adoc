...
A migration's idle timer resets each time you xref:sideloader:migrate-sideloader.adoc#upload-snapshots-to-migration-directory[upload snapshots] or xref:sideloader:migrate-sideloader.adoc#import-data[import data].

You can you can also override the idle timer by manually starting a cleanup or scheduling a cleanup.
However, _you cannot permanently prevent the cleanup process_.
