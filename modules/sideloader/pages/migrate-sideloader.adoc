...
You need to be familiar with using the command line, including installing and using CLI tools, issuing curl commands, basic scripting, and security best practices.

This process uses authentication credentials to write to the migration directory and your database.
Make sure you understand how to securely store and use sensitive credentials.

[#create-snapshots]
== Create snapshots

On _each node_ in your origin cluster, use `nodetool snapshot` to create a backup of the data that you want to migrate, including all keyspaces and CQL tables that you want to migrate.
...
. If your origin cluster has xref:dse-5.1@cql:develop:indexing/2i/2i-concepts.adoc[secondary indexes], remove all directories related to those indexes from all snapshots before you xref:sideloader:migrate-sideloader.adoc#upload-snapshots-to-migration-directory[upload the snapshots].
+
[WARNING]
====
...
If you use automation to handle {sstable-sideloader} migrations, you might need to scheduled a xref:sideloader:stop-restart-sideloader.adoc[pause] every hour so you can generate new credentials without unexpectedly interrupting the migration.
====
--
...
{company} recommends that you xref:sideloader:cleanup-sideloader.adoc[manually reschedule the cleanup] if you don't plan to launch the migration within one week or if you need several days to upload snapshots or import data.

[WARNING]
====
For large migrations, it can take several days to upload snapshots and import data.
Make sure you xref:sideloader:cleanup-sideloader.adoc[manually reschedule the cleanup] to avoid automatic cleanup.
====
...
Wait a few minutes before you check the status again.
* `"status": "MigrationDone"`: The import is complete, and you can proceed with validating the data.
