...
Before you use {sstable-sideloader} for a migration, do the following:

* xref:sideloader:sideloader-overview.adoc[Learn about the {sstable-sideloader} process]
* xref:sideloader:prepare-sideloader.adoc[Prepare your environments for {sstable-sideloader}]
...
For this reason, if you need to delay your migration, {company} recommends that you override automatic cleanup by xref:sideloader:cleanup-sideloader.adoc[scheduling a cleanup] to run at a date and time that better aligns with your overall migration plan.
