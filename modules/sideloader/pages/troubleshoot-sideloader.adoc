...
The option you use depends on where the error occurred.

* *Schema discrepancies*: This failure occurs when there is a mismatch between the origin and target schemas.
+
...
+
For this option, see <<relaunch>>.

* *Invalid data in migration directory*: This failure occurs when there is a problem with the data uploaded to the migration director.
For example, the data might be corrupt, incomplete due a timeout, or include invalid data such as secondary index directories.
+
When this type of failure occurs, you must reset the migration and restart the entire migration process.
+
For this option, see <<reset>>.
...
+
Some errors can't be resolved after the migration has started.
...
[#reset]
=== Reset a failed migration

////
Future: Reset failed migration endpoint
