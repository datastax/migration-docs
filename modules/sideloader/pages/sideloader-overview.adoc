...
Migration::
A workflow that you initiate within {sstable-sideloader} that encompasses the lifecycle of uploading and importing snapshot backups of a specific set of keyspaces or {cql-short} tables.
+
...
Each snapshot for each node in the origin cluster must include all the keyspaces and individual {cql-short} tables that you want to migrate.
