...
These snapshots are ideal for database migrations because creating snapshots has a negligible performance impact on the origin cluster, and the snapshots preserve metadata like write timestamps and expiration times (TTLs).

When using {sstable-sideloader} with {product-proxy}, the preserved timestamps ensure that new, real-time writes accurately take precedence over historical writes.
