= About {astra-db} {sstable-sideloader}
...
For more information, see xref:sideloader:migrate-sideloader.adoc#create-snapshots[Migrate data with {sstable-sidealoder}: Create snapshots].
...
For specific requirements and more information, see xref:sideloader:migrate-sideloader.adoc#record-schema[Migrate data with {sstable-sidealoder}:Configure the target database].
...
While it is possible to orchestrate this process through a staging server, this commands given in this documentation assume you are uploading snapshots directly from the origin cluster.
