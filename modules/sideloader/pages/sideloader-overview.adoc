...
include::ROOT:partial$astra-role-attributes.adoc[]
:description: {sstable-sideloader} lets you migrate data from an {cass-reg} or {dse} cluster into {astra-db} without impacting the origin cluster or your {product} database.
:loop-var: pass:[${i}]
...
[TIP]
====
If you need to migrate a live database, you can use {sstable-sideloader} instead of DSBulk or {cass-short} Data Migrator during Phase 2 (data-loading phase) of xref:data-migration:ROOT:introduction.adoc[Zero Downtime Migration (ZDM)].
...
In the context of {sstable-sideloader}, this refers to the {product} database where you will migrate your data.

Administration server::
A server where you can run the migration commands, including CLI commands and {product-short} {devops-api} calls.
...
[%collapsible]
====
After you create snapshots on the origin cluster and pre-configure the schema on the target database, you can <<initialize-migration,use the {product-short} {devops-api} to initialize the migration>>.
...
+
{company} owns the migration bucket, and it is located within the {product-short} perimeter.
...
In this case, consider creating your target database in a co-located datacenter, and then xref:databases:manage-regions.adoc[deploy your database to other regions] after the migration.
|===
====
...
However, in the context of xref:data-migration:ROOT:introduction.adoc[Zero Downtime Migration], it's important that the ZDM proxy continues to read from the origin cluster.

. Re-enables compactions on the {product} database.
...
* Your {astra-db} organization must be on an *Enterprise* xref:administration:subscription-plans.adoc[subscription plan].
+
{sstable-sideloader} is a premium feature that incurs costs based on usage.

* Your target database must be a {product} database.
If you don't already have one, xref:databases:create-database.adoc[create a database].
...
////
Uncomment for GA:
* Your target database must be in a xref:administration:provisioned-capacity-units.adoc[PCU group].
+
PCU groups can support either xref:administration:provisioned-capacity-units.adoc#workload-def[committed capacity or flexible capacity workloads].
+
Committed capacity PCU groups require a long-term commitment to a minimum amount of reserved capacity.
They are intended for databases that require continuous availability for a long time period (upwards of one year or more), such as latency-sensitive production workloads.
In contrast, flexible capacity PCU groups can scale to zero when not needed, making them a cost-effective option for temporary capacity requirements, such as {sstable-sideloader} operations.
+
Because {sstable-sideloader} operations are typically short-term events that are potentially resource-intensive, {company} recommends that you xref:administration:provisioned-capacity-units.adoc#create-pcu-groups[[create a flexible capacity PCU group] exclusively for your target database:
+
** When creating PCU groups in the {astra-ui}, select *Flexible capacity workload* for the *Workload type*.
** When creating PCU groups with the {devops-api}, set `reserve` to 0 and `min` to 1 or more.
+
After the data transfer, you can remove your target database from the flexible capacity PCU group, and then park or delete the group to minimize costs.
However, don't park the PCU group during the {sstable-sideloader} process.
Databases in a parked PCU group are hibernated and unavailable for use.
//TODO: Add to PCU user stories.
////
...
* https://jqlang.github.io/jq/[jq] to format JSON responses from the {product-short} {devops-api}.
...
----
//However, {sstable-sideloader} can't import data to a xref:databases:collection in a {db-serverless-vector} database.
. Recreate the schemas in your target database:
+
.. In the {astra-ui-link}, go to *Databases*, and then select your xref:databases:create-database.adoc#create-a-serverless-non-vector-database[{db-serverless} database].
.. xref:databases:manage-keyspaces.adoc#keyspaces[Create a keyspace] with the exact same name as your origin cluster's keyspace.
.. In your database's xref:cql:develop-with-cql.adoc#connect-to-the-cql-shell[CQL console], create tables with the exact same names and schemas as your origin cluster.
+
...
See xref:databases:database-limits.adoc[] for more information.
...
----
+
Replace *`DATABASE_ID`* with the xref:databases:create-database.adoc#get-db-id[database ID], and replace *`TOKEN`* with an xref:administration:manage-application-tokens.adoc[application token] with the *Database Administrator* role.
+
[TIP]
====
