...
For additional assistance, you can <<report-an-issue,report an issue>>, contact your {company} account representative, or contact {support-url}[IBM Support].
...
If you observe this behavior in your logs, <<report-an-issue,report an issue>> so the issue can be investigated by the {zdm-short} team.
...
For example, in the Java driver, you can set `{java-driver-url}/blob/4.x/core/src/main/resources/reference.conf#L1365[advanced.monitor-reporting]` to `false`.
...
If you find an error like this, <<report-an-issue,report an issue>> so the {zdm-short} team can investigate it.
...
* Your <<check-version,{zdm-proxy} version>>.

* <<proxy-logs,{zdm-proxy} logs>>, ideally at `DEBUG` level, if you can easily reproduce the issue and tolerate restarting the proxy instances to apply the log level configuration change.
