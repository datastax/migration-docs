...
However, this can slightly degrade performance, and {company} recommends that you revert to `INFO` logging as soon as you possible.

How you set the log level depends on how you deployed the {product-proxy}:

* If you used {product-automation} to deployed the {product-proxy}, set `log_level` in `vars/zdm_proxy_core_config.yml`.
+
You can change this value in a rolling fashion by editing this variable and running the `rolling_update_zdm_proxy.yml` playbook.
...
If you used the {product-automation} to deploy {product-proxy}, then you can get logs for a single proxy instance and use a playbook to retrieve logs for all instances.
...
However, if you enable `DEBUG` logging, `debug` messages can help you track the source of a problem.
...
If the meaning of a `warn` or `error` message isn't clear, you can submit an issue in the https://github.com/datastax/zdm-proxy/issues[{product-proxy} GitHub repository].

=== Common log messages

Here are the most common messages in the proxy logs.

==== {product-proxy} startup message

If the log level doesn't filtering out `INFO` entries, you can look for a `Proxy started` log message to verify that the {product-proxy} started correctly.
For example:
...
If the log level doesn't filtering out `INFO` entries, the first few lines of a {product-proxy} log file contain all configuration variables and values in a long JSON string.
...
* https://github.com/datastax/zdm-proxy/issues[{product-proxy} repository]
* https://github.com/datastax/zdm-proxy-automation/issues[{product-automation} repository] (includes {product-automation} and the {product-utility})
* https://github.com/datastax/cassandra-data-migrator/issues[{cass-migrator} repository]
* https://github.com/datastax/dsbulk-migrator/issues[{dsbulk-migrator} repository]
