...
The contact point is the first point of contact to the cluster, but the driver discovers the rest of the nodes through {cql-short} queries.
...
** Are you passing a `BEGIN BATCH` {cql-short} query string to a simple/prepared statement, or do you use the actual batch statement objects that the drivers allow you to create?

* How many parameters does each statement have?

* Is {cql-short} function replacement enabled?
