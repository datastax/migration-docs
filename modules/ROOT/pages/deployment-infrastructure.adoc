...
To use the following template, replace all placeholders (in angle brackets) with the appropriate values for your deployment, and add more entries if you have more than three proxy instances.
...
Host *
    User <linux user>
    IdentityFile <Filename and absolute path to the locally generated key pair for the ZDM infrastructure, such as ~/.ssh/zdm-key-XXX>
    IdentitiesOnly yes
