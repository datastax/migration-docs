...
== Error with {astra-db} DevOps API when using the {product-automation}
...
The {astra-db} DevOps API is likely temporarily unavailable.
...
If a client application is only sending reads through a connection then the Target (i.e. {astra-db} in this case) connection will not get any traffic because {product-short} forwards all reads to the Origin connection.
...
* {astra-db} directly
* Origin directly
* {product-short} (with {astra-db} and Origin)

The results of these tests show latency/throughput values are worse with {product-short} than when connecting to {astra-db} or Origin directly.
...
This means that if Origin has better performance than Target then {product-short} will inevitably have a worse performance for writes.
