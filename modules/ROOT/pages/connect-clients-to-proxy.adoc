...
If your {product-proxy} is configured to use {astra-db} as the origin or target cluster, then your client application doesn't need to provide a Secure Connect Bundle (SCB) when connecting to the proxy.

As an alternative to providing the SCB directly, you can xref:astra-db-serverless:administration:manage-application-tokens.adoc[generate an application token] with the *Organization Administrator* role, and then specify one of the following sets of credentials generated with the token:
...
** The {astra}-compatible version additionally supports connecting directly to an {astra-db} cluster by passing the cluster's Secure Connect Bundle and valid credentials.
