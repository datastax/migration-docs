...
After you edit mutable variables in their corresponding configuration files (`vars/zdm_proxy_core_config.yml`, `vars/zdm_proxy_cluster_config.yml`, or `vars/zdm_proxy_advanced_config.yml`), you must <<perform-a-rolling-restart-of-the-proxies,perform a rolling restart>> to apply the configuration changes to your {zdm-proxy} instances.
...
. <<perform-a-rolling-restart-of-the-proxies,Perform a rolling restart>> to update all {zdm-proxy} instances to the new version.
