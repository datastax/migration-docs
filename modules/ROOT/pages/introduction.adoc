...
From a functional standpoint, before you begin a migration, your client applications perform read/write operations directly with your existing xref:cql:ROOT:index.adoc[{cql-short}]-compatible database, such as {cass}, {dse-short}, {hcd-short}, or {astra-db}.
