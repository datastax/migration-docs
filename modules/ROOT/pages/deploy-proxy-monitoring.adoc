...
** For an {astra-db} database, provide either `*_astra_secure_connect_bundle_path` _or both_ `*_astra_db_id` and `*_astra_token`.
...
*** If you want to manually upload your database's SCB to the jumphost, use `*_astra_secure_connect_bundle_path`.
+
...
. Open a new shell on the jumpost, and then run `docker cp /path/to/scb.zim zdm-ansible-container:/home/ubuntu` to copy the SCB to the container.
. Set `*_astra_secure_connect_bundle_path` to the path to the SCB on the jumphost.
====
