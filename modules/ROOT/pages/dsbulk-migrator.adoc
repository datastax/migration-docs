...
| 
| `--import-bundle=PATH`
| The path to a secure connect bundle to connect to the Target cluster, if it's a {company} {astra-db} cluster.
...
Migrate live from an existing Origin cluster to a Target {astra-db} cluster using an external {dsbulk-loader} installation. 
...
Migrate live from an existing Origin cluster to a Target {astra-db} cluster using the embedded {dsbulk-loader} installation. 
Passwords will be prompted interactively. 
In this example, additional {dsbulk-loader} options are passed. 
...
Otherwise, an error will be raised because no embedded {dsbulk-loader} can be found.
====
