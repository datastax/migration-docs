...
Note that CQL statements, such as those used to reproduce the schema on the target database, can be executed in Astra DB
in two ways:

. the Astra DB interface offers a CQL Console directly https://docs.datastax.com/en/astra-serverless/docs/connect/cql/connect-cqlsh.html#_embedded_astra_db_cql_shell[in the browser^];
. alternatively, you can connect remotely from any machine as long as you use an Astra DB-compatible https://docs.datastax.com/en/astra-serverless/docs/connect/cql/connect-cqlsh.html#_standalone_cql_shell[standalone `cqlsh` client^] (and provide the necessary connection secrets).
