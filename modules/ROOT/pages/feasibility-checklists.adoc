...
[TIP]
====
Ensure that you test your client application with Target (connected directly without the {product-proxy}) before the migration process begins.
====
...
A potential workaround is to not connect these read-only client applications to {product-proxy}, but you need to ensure that these client applications switch reads to Target at any point after all the data has been migrated and all validation and reconciliation has completed.
...
When {product-proxy} receives a response from both Origin and Target, each response would have its own `applied` flag. 
...
To reiterate, {product-proxy} only returns the `applied` value from the primary cluster, which is the cluster from where read results are returned to the client application (by default, Origin).
...
Read-only Search workloads can be moved directly from Origin to Target without {product-proxy} being involved.
...
The authentication configuration on each cluster can be different between Origin and Target, as the {product-proxy} treats them independently.
