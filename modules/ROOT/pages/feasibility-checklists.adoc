...
When you introduce {product-proxy}, the target cluster is integrated into the protocol negotiation process to ensure that the negotiated protocol version is supported by the origin cluster, target cluster, and the driver.
====
...
+
If you plan to upgrade to a `V5`-compatible driver before or during your migration, the potential performance impact depends on which clusters support `V5`:
+
--
...
* **Driver configuration**: You can explicitly downgrade the protocol version in your client application's driver configuration.
...
* **{product-proxy} configuration**: You can use the `xref:ROOT:deploy-proxy-monitoring.adoc#blocked-protocol-versions[blocked_protocol_versions]` configuration variable to block specific protocol versions at the proxy level.
