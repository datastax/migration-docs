...
`CL.ONE` isn't supported by {astra}, and xref:ROOT:components.adoc#how-zdm-proxy-handles-reads-and-writes[read and write requests sent through {product-proxy}] with `CL.ONE` to {astra-db} databases always fail.
...
The xref:astra-db-serverless:api-reference:compare-dataapi-to-stargate.adoc[Stargate APIs] (Document, REST, GraphQL, gRPC) are deprecated for {astra}.
