...
This removes the dependency on {zdm-proxy} and the origin cluster, thereby completing the migration process.
...
The following two pseudocode examples provide a simplified comparison of the way a {cass-short} driver interacts with {astra-db} and self-managed {cass-short} clusters.
...
When you are ready, you can decommission your origin cluster and {zdm-proxy} because these are no longer needed and xref:ROOT:rollback.adoc[seamless rollback] is no longer possible.
