...
* Phase 1: Connected client applications to {product-proxy}, which included setting up Ansible playbooks with the {product-proxy}, and deploying the {product-proxy} instances via the Docker container with {zdm-automation}.
...
** Note: You will already have used these credentials when you configured the {product-proxy} to connect to your Astra DB cluster as Target.
