...
Before running {product-utility} to create the Ansible Control Host contain, you must complete the following prerequisites:
...
This is a distruptive operation that requires a small amount of downtime because the deployment playbook decommissions and recreates all {product-proxy} containers simultaneously.
...
The jumphost serves three purposes:
...
The following steps assume that this file exists.
====
...
[IMPORTANT]
====
{company} strongly recommendeds that you configure a monitoring instance to expose xref:ROOT:metrics.adoc[{product-proxy} metrics and preconfigured dashboards] with {product-automation}.
